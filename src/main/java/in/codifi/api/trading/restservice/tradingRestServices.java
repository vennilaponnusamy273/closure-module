...
public class tradingRestServices {
	...
	private void handleJsonParsingError(String authToken, JSONException e) {
	    ...
	    e.printStackTrace();
	    commonMethods.SaveLog(authToken, "tradingRestServices", "getFunds", e.getMessage());
	    commonMethods.sendErrorMail(
	            "An error occurred while processing your request, tradingRestServices In getFunds for the Error: "
	                    + e.getMessage(),
	            "ERR-001"
	    );
	}
	...
	public boolean getHoldings(String authToken) {
	    ...
	    try {
	            ...
	            if ("Ok".equals(jsonResponse.getString("status"))) {
	                if ("no data".equalsIgnoreCase(jsonResponse.getString("message"))) {
	                    ...
	                    if (resultArray.length() == 0) {
	                        ...
	                        // "status" is "Ok", "message" is "no data", but "result" is not empty, return true
	                        return true;
	                    }
	                } else {
	        ...
	        System.err.println("JSON parsing error: " + e.getMessage());
	        // JSON parsing error, return false
	        return false;
	    }
	}
		...
		try {
	        if (PositionResult.getStatus() == 200) {
	            ...
	            if (!"Not ok".equalsIgnoreCase(jsonResponse.getString("status"))) {
	                ...
	                return false; // "status" is "Not ok" and message is "No data found for Positions", return false
	            } else {
	                return true; // "status" is "Not ok" but message is not "No data found for Positions", return true
	            }
	        } else {
